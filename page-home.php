<?php

/**
 * The header.

 * @package WordPress
 * @subpackage lyc
 * @since lyc 1.0 1.0
 */

get_header();
?>


<main>
  <!-- Banner -->
  <section class="lyc-banner">
    <div class="container-fluid p-0">
      <div class="row">
        <div class="col-xxl-1 col-xl-1 col-lg-1 col-md-0 col-sm-0 col-0">
          <div class="lyc-linkedin-button lyc-linkedin-button d-xxl-flex d-xl-flex d-lg-flex d-md-none d-sm-none d-none justify-content-center text-center w-100 bg-info">
            <a href="#" class="lyc-linkedin-btn btn bg-info border-info">
              LinkedIn
            </a>
          </div>
        </div>
        <div class="col-xxl-1 col-xl-1 col-lg-1 col-md-0 col-sm-0 col-0 d-xxl-flex d-xl-flex d-lg-flex d-md-none d-sm-none d-none justify-content-center align-items-center">
          <p class="lyc-cons">counselling</p>
        </div>
        <div class="col-xxl-10 col-xl-10 col-lg-10 col-md-12 col-sm-12 col-12">
          <div class="lyc-banner-content d-flex justify-content-center align-items-center p-xxl-20 p-xl-20 p-lg-10 p-md-5 p-sm-3 p-3 my-xxl-10 my-xl-10 my-lg-7 my-md-0 my-sm-0 my-0">
            <div class="lyc-banner-content-text d-flex flex-column justify-content-start align-items-start">
              <a href="#" class="btn btn-secondary">London Young Counselling</a>
              <h1>At the heart of <span class="lyc-child">children</span> & <span class="lyc-young"> Young </span> peoples mental health</h1>
            </div>
          </div>
        </div>
      </div>
    </div>
  </section>
  <!-- Banner -->
  <!-- Afterbanner -->
  <section class="lyc-after-banner">
    <div class="row">
      <?php
        if (have_rows('after_banner_box')) : ?>
      <?php while (have_rows('after_banner_box')) : the_row();
            $heading = get_sub_field('heading');
            $content = get_sub_field('content');
            $icon = get_sub_field('icon');
          ?>
      <div class="col-xxl-4 col-xl-4 col-lg-12 col-md-12 col-sm-12 col-12">
        <div class="lyc-after-banner-box d-flex flex-column lyc-after-banner-box<?php echo get_row_index(); ?> h-100 p-xxl-5 p-xl-5 p-lg-4 p-md-4 p-sm-3 p-3 gap-xxl-4 gap-xl-4 gap-lg-3 gap-md-3 gap-sm-3 gap-3">
          <div class="lyc-box-heading d-flex align-items-center flex-wrap gap-xxl-4 gap-xl-4 gap-lg-3 gap-md-3 gap-sm-3 gap-3">
            <?php
                     $formatted_text = str_replace(['<p>', '</p>'], '', $icon);
                    //  echo esc_html($formatted_text);
                       ?>
            <?php echo htmlspecialchars_decode($formatted_text); ?>
            <h3><?php echo $heading;  ?></h3>
          </div>
          <p><?php echo $content;  ?></p>
        </div>
      </div>
      <?php endwhile; ?>
      <?php endif; ?>
    </div>
  </section>
  <!-- Afterbanner -->
  <!-- Why imp -->
  <section class="lyc-why-imp my-xxl-7 my-xl-7 my-lg-5 my-md-5 my-sm-3 my-3">
    <div class="container">
      <?php if( have_rows('why_us') ): ?>
      <?php while( have_rows('why_us') ): the_row();

        $title = get_sub_field('title');
        $heading = get_sub_field('heading');
        $image = get_sub_field('image');

        ?>
      <div class="row">
        <div class="col-xxl-4 col-xl-4 col-lg-12 col-md-12 col-sm-12 col-12">
          <div class="lyc-why-imp-img lyc-why-imp-img w-100 d-flex justify-content-center align-items-center">
            <?php
                if (!empty($image)) : ?>
            <img src="<?php echo esc_url($image['url']); ?>" title="<?php echo esc_attr($image['alt']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" class="img-fluid" width="600" height="600">
            <?php endif; ?>
          </div>
        </div>
        <div class="col-xxl-8 col-xl-8 col-lg-12 col-md-12 col-sm-12 col-12 d-flex justify-content-center align-items-center">
          <div class="lyc-why-imp-content ps-xxl-5 ps-xl-5 ps-lg-4 ps-md-3 ps-sm-0 ps-0 d-flex flex-column gap-xxl-4 gap-xl-4 gap-lg-4 gap-md-3 gap-sm-3 gap-3">
            <div class="lyc-heading d-flex flex-column gap-4">
              <span><?php echo $title; ?></span>
              <h2><?php echo $heading; ?></h2>
            </div>
            <div class="lyc-why-imp-content-text">
              <div class="row">
                <?php
                    if (have_rows('box')) : ?>
                <?php while (have_rows('box')) : the_row();
                        $icon = get_sub_field('icon');
                        $title = get_sub_field('title');
                        $content = get_sub_field('content');

                    ?>
                <!-- Box1 -->
                <div class="col-xxl-4 col-xl-4 col-lg-4 col-md-4 col-sm-12 col-12">
                  <div class="lyc-why-imp-content-text-box d-flex flex-column align-items-start gap-2 p-4 bg-white h-100">
                    <div class="lyc-box-icon p-3 d-flex justify-content-center align-items-center">
                      <?php
                             $formatted_text = str_replace(['<p>', '</p>'], '', $icon);
                            //  echo esc_html($formatted_text);
                         ?>
                      <?php echo htmlspecialchars_decode($formatted_text); ?>
                    </div>
                    <h3><?php echo $title;  ?></h3>
                    <p><?php echo $content;  ?></p>
                  </div>
                </div>
                <?php endwhile; ?>
                <?php endif; ?>
              </div>
            </div>
          </div>
        </div>
      </div>
      <?php endwhile; ?>
      <?php endif; ?>
    </div>
  </section>
  <!-- Why imp -->
  <!-- What we do -->
  <section class="lyc-what-we-do py-xxl-7 py-xl-7 py-lg-5 py-md-5 py-sm-3 py-3 bg-primary-500 position-relative">
    <div class="container">
      <div class="lyc-what-we-do-content d-flex flex-column gap-xxl-5 gap-xl-5 gap-lg-4 gap-md-4 gap-sm-3 gap-2 position-relative">
        <?php if( have_rows('what_we_do') ): ?>
        <?php while( have_rows('what_we_do') ): the_row();
                $title = get_sub_field('title');
                $heading = get_sub_field('heading');
                ?>
        <div class="lyc-heading d-flex flex-column gap-4 justify-content-center align-items-center text-center">
          <span><?php echo $title;  ?></span>
          <h2><?php echo $heading;  ?></h2>
        </div>
        <?php endwhile; ?>
        <?php endif; ?>
        <div class="lyc-what-we-do-content-text">
          <?php
                $args = array(
                    'numberposts' => 3,
                    'post_type'   => 'what_we_do',
                    'post_status'     => 'publish',
                    'order' => 'ASC',
                );
                $whats = get_posts($args);
                if (count($whats) > 0) {
                ?>
          <div class="row">
            <!-- Box1 -->
            <?php
                    foreach ($whats as $what) {
                        $image = wp_get_attachment_image_src(get_post_thumbnail_id($what->ID), 'single-post-thumbnail');
                        $image_alt =  get_post_meta(get_post_thumbnail_id($what->ID), '_wp_attachment_image_alt', true);
                    ?>
            <div class="col-xxl-4 col-xl-4 col-lg-4 col-md-12 col-sm-12 col-12">
              <div class="lyc-what-we-do-text-box w-100 d-flex flex-column gap-4">
                <div class="lyc-box-img w-100">
                  <img src="<?php echo $image[0]; ?>" width="523" height="400" title="<?php echo $image_alt; ?>" alt="<?php echo $image_alt; ?>" class="img-fluid w-100">
                </div>
                <div class="lyc-box-content d-flex flex-column align-items-start justify-content-center h-100 gap-3">
                  <div class="lyc-box-text d-flex flex-column gap-2">
                    <h4><?php echo $what->post_title; ?></h4>
                    <p><?php echo $what->post_excerpt; ?></p>
                  </div>
                  <a href="<?php echo get_permalink($what->ID); ?>" class="btn d-flex align-items-center gap-2">Read More
                    <svg width="12" height="13" viewBox="0 0 12 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                      <g clip-path="url(#clip0_78_357)">
                        <mask id="mask0_78_357" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="12" height="13">
                          <path d="M12 0.5H0V12.5H12V0.5Z" fill="white" />
                        </mask>
                        <g mask="url(#mask0_78_357)">
                          <path d="M10.0034 3.90831L1.41176 12.5L0 11.0882L8.59169 2.49654H1.01905V0.5H12V11.481H10.0034V3.90831Z" fill="#004AAD" />
                        </g>
                      </g>
                      <defs>
                        <clipPath id="clip0_78_357">
                          <rect width="12" height="12" fill="white" transform="translate(0 0.5)" />
                        </clipPath>
                      </defs>
                    </svg>
                  </a>
                </div>
              </div>
            </div>
            <?php } ?>
          </div>
          <?php } ?>
        </div>
      </div>
    </div>
    <div class="lyc-background position-absolute bottom-0">
      <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/media/what-do/background.png" title="what-do" alt="what-do" class="img-fluid" width="1920" height="554">
    </div>
  </section>
  <!-- What we do -->
  <!-- About us-->
  <section class="lyc-about-us pb-xxl-7 pb-xl-7 pb-lg-5 pb-md-5 pb-sm-3 pb-3">
    <div class="container">
      <?php if( have_rows('about_us') ): ?>
      <?php while( have_rows('about_us') ): the_row();

        $title = get_sub_field('title');
        $heading = get_sub_field('heading');
        $content = get_sub_field('content');
        $school_button = get_sub_field('school_button');
        $about_button = get_sub_field('about_button');
        $extra_content = get_sub_field('extra_content');
        $side_content = get_sub_field('side_content');
        $image = get_sub_field('image');

        ?>
      <div class="row">
        <div class="col-xxl-6 col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12 p-0">
          <div class="lyc-about-us-content d-flex flex-column justify-content-center gap-xxl-4 gap-xl-4 gap-lg-4 gap-md-3 gap-sm-3 gap-3 p-xxl-5 p-xl-5 p-lg-4 p-md-4 p-sm-3 p-3 position-relative h-100">
            <div class="lyc-heading">
              <span><?php echo $title; ?></span>
              <h2><?php echo $heading; ?></h2>
            </div>
            <p><?php echo $content; ?></p>
            <h6><?php echo $extra_content; ?></h6>
            <div class="lyc-about-us-content-btn d-flex flex-wrap align-items-center gap-xxl-3 gap-xl-3 gap-lg-3 gap-md-2 gap-sm-2 gap-2">
              <a href="<?php echo esc_url( $school_button['url'] ); ?>" class="btn d-flex align-items-center gap-2">School Counselling Service
                <svg width="12" height="13" viewBox="0 0 12 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <g clip-path="url(#clip0_78_357)">
                    <mask id="mask0_78_357" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="12" height="13">
                      <path d="M12 0.5H0V12.5H12V0.5Z" fill="white"></path>
                    </mask>
                    <g mask="url(#mask0_78_357)">
                      <path d="M10.0034 3.90831L1.41176 12.5L0 11.0882L8.59169 2.49654H1.01905V0.5H12V11.481H10.0034V3.90831Z" fill="#004AAD"></path>
                    </g>
                  </g>
                  <defs>
                    <clipPath id="clip0_78_357">
                      <rect width="12" height="12" fill="white" transform="translate(0 0.5)"></rect>
                    </clipPath>
                  </defs>
                </svg>
              </a>
              <a href="<?php echo esc_url( $about_button['url'] ); ?>" class="btn d-flex align-items-center gap-2 btn1">About Us
                <svg width="12" height="13" viewBox="0 0 12 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                  <g clip-path="url(#clip0_78_357)">
                    <mask id="mask0_78_357" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="12" height="13">
                      <path d="M12 0.5H0V12.5H12V0.5Z" fill="white"></path>
                    </mask>
                    <g mask="url(#mask0_78_357)">
                      <path d="M10.0034 3.90831L1.41176 12.5L0 11.0882L8.59169 2.49654H1.01905V0.5H12V11.481H10.0034V3.90831Z" fill="#004AAD"></path>
                    </g>
                  </g>
                  <defs>
                    <clipPath id="clip0_78_357">
                      <rect width="12" height="12" fill="white" transform="translate(0 0.5)"></rect>
                    </clipPath>
                  </defs>
                </svg>
              </a>
            </div>
            <div class="lyc-background position-absolute top-0 bottom-0 start-0 end-0 h-100">
              <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/media/about/pattern.webp" title="pattern" alt="pattern" class="img-fluid h-100" width="1182" height="777">
            </div>
          </div>
        </div>
        <div class="col-xxl-6 col-xl-6 col-lg-6 col-md-12 col-sm-12 col-12 p-0">
          <div class="lyc-about-us-img h-100 position-relative w-100">
            <?php
                if (!empty($image)) : ?>
            <img src="<?php echo esc_url($image['url']); ?>" title="<?php echo esc_attr($image['alt']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" class="img-fluid h-100 w-100" width="800" height="700">
            <?php endif; ?>
            <div class="lyc-rect position-absolute bg-primary-500 py-xxl-5 py-xl-5 py-lg-5 py-md-4 py-sm-3 py-3 px-xxl-4 px-xl-4 px-lg-4 px-md-3 px-sm-2 px-2">
              <h4><?php echo $side_content; ?></h4>
            </div>
          </div>
        </div>
      </div>
      <?php endwhile; ?>
      <?php endif; ?>
    </div>
  </section>
  <!-- About us -->
  <!-- work process -->
  <section class="lyc-work-prc py-xxl-7 py-xl-7 py-lg-5 py-md-5 py-sm-3 py-3">
    <?php if( have_rows('work_process') ): ?>
    <?php while( have_rows('work_process') ): the_row();
        $title = get_sub_field('title');
        $heading = get_sub_field('heading');
        ?>
    <div class="container">
      <div class="lyc-heading my-xxl-5 my-xl-5 my-lg-4 my-md-4 my-sm-3 my-2 ">
        <span><?php echo $title; ?></span>
        <h2><?php echo $heading; ?></h2>
      </div>
      <div class="lyc-work-prc-content">
        <div class="row">
          <?php
                    if (have_rows('box')) : ?>
          <?php while (have_rows('box')) : the_row();
                        $icon = get_sub_field('icon');
                        $title = get_sub_field('title');
                        $content = get_sub_field('content');
                    ?>
          <!-- Box1 -->
          <div class="col-xxl-3 col-xl-3 col-lg-3 col-md-6 col-sm-12 col-12">
            <div class="lyc-work-prc-box d-flex flex-column justify-content-center align-items-center gap-xxl-4 gap-xl-4 gap-lg-4 gap-md-3 gap-sm-2 gap-2">
              <div class="lyc-box-icon d-flex justify-content-center align-items-center position-relative">
                <?php
                             $formatted_text = str_replace(['<p>', '</p>'], '', $icon);
                            //  echo esc_html($formatted_text);
                         ?>
                <?php echo htmlspecialchars_decode($formatted_text); ?>
                <span class="bg-secondary d-flex justify-content-center align-items-center text-white position-absolute top-0 end-0"><?php echo get_row_index(); ?></span>
              </div>
              <h4><?php echo $title;  ?></h4>
              <p class="text-center"><?php echo $content;  ?></p>
            </div>
          </div>
          <?php endwhile; ?>
          <?php endif; ?>
        </div>
      </div>
    </div>
    <?php endwhile; ?>
    <?php endif; ?>
  </section>
  <!-- work process -->
  <!-- Why-choose-us -->
  <section class="lyc-why-choose-us mt-xxl-7 mt-xl-7 mt-lg-5 mt-md-5 mt-sm-3 mt-3">
    <?php if( have_rows('why_choose_us') ): ?>
    <?php while( have_rows('why_choose_us') ): the_row();

        $image = get_sub_field('image');
        $title = get_sub_field('title');
        $content = get_sub_field('content');
        $sub_title = get_sub_field('sub_title');
        $our_imapact_title = get_sub_field('our_imapact_title');
        $after_one_title = get_sub_field('after_one_title');

        ?>
    <div class="row d-flex justify-content-center align-items-center">
      <div class="col-xxl-5 col-xl-5 col-lg-6 col-md-12 col-sm-12 col-12">
        <div class="lyc-why-choose-us-img">
          <?php
                if (!empty($image)) : ?>
          <img src="<?php echo esc_url($image['url']); ?>" title="<?php echo esc_attr($image['alt']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" width="800" height="756" class="img-fluid">
          <?php endif; ?>
        </div>
      </div>
      <div class="col-xxl-7 col-xl-7 col-lg-6 col-md-12 col-sm-12 col-12">
        <div class="lyc-why-choose-us-text d-flex flex-column gap-xxl-4 gap-xl-4 gap-lg-4 gap-md-3 gap-sm-3 gap-2 px-xxl-5 px-xl-5 px-lg-4 px-md-3 px-sm-3 px-3">
          <div class="lyc-heading d-flex flex-column gap-xxl-4 gap-xl-4 gap-lg-4 gap-md-3 gap-sm-3 gap-2">
            <span><?php echo $title; ?></span>
            <h2><?php echo $sub_title; ?></h2>
            <p><?php echo $content; ?></p>
          </div>
          <!-- row1 -->
          <div class="lyc-row1 d-flex flex-column gap-xxl-4 gap-xl-4 gap-lg-4 gap-md-3 gap-sm-3 gap-2">
            <h4><?php echo $our_imapact_title; ?></h4>
            <div class="row">
              <!-- Box1 -->
              <?php
                    if (have_rows('our_impact')) : ?>
              <?php while (have_rows('our_impact')) : the_row();
                        $title = get_sub_field('title');
                        $content = get_sub_field('content');
                    ?>
              <div class="col-xxl-4 col-xl-4 col-lg-6 col-md-6 col-sm-12 col-12">
                <div class="lyc-choose-box">
                  <span class="bg-white"><?php echo $title; ?></span>
                  <p class="p-4"><?php echo $content; ?></p>
                </div>
              </div>
              <?php endwhile; ?>
              <?php endif; ?>
            </div>
          </div>
          <!-- row2 -->
          <div class="lyc-row1 d-flex flex-column gap-xxl-4 gap-xl-4 gap-lg-4 gap-md-3 gap-sm-3 gap-2">
            <h4><?php echo $after_one_title; ?></h4>
            <div class="row">
              <!-- Box1 -->
              <?php
                    if (have_rows('after')) : ?>
              <?php while (have_rows('after')) : the_row();
                        $title = get_sub_field('title');
                        $content = get_sub_field('content');
                    ?>
              <div class="col-xxl-4 col-xl-4 col-lg-6 col-md-6 col-sm-12 col-12">
                <div class="lyc-choose-box">
                  <span class="bg-white"><?php echo $title; ?></span>
                  <p class="p-4"><?php echo $content; ?></p>
                </div>
              </div>
              <?php endwhile; ?>
              <?php endif; ?>
            </div>
          </div>
        </div>
      </div>
    </div>
    <?php endwhile; ?>
    <?php endif; ?>
  </section>
  <!-- Why-choose-us -->
  <!-- Members -->
  <section class="lyc-members my-xxl-7 my-xl-7 my-lg-5 my-md-5 my-sm-3 my-3">
    <div class="container">
      <?php if( have_rows('expert_member') ): ?>
      <?php while( have_rows('expert_member') ): the_row();

        $title = get_sub_field('title');
        $sub_title = get_sub_field('sub_title');

        ?>
      <div class="lyc-members-heading d-flex flex-wrap gap-3 justify-content-between align-items-center lyc-heading mb-xxl-5 mb-xl-5 mb-lg-4 mb-md-4 mb-sm-3 mb-2 ">
        <div class="lyc-heading">
          <span><?php echo $title; ?></span>
          <h2><?php echo $sub_title; ?></h2>
        </div>
        <a href="<?php echo site_url('staff'); ?>" class="btn btn-primary d-flex flex-wrap justify-content-center align-items-center gap-2">
          View all members
          <svg width="12" height="13" viewBox="0 0 12 13" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_78_357)">
              <mask id="mask0_78_357" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="12" height="13">
                <path d="M12 0.5H0V12.5H12V0.5Z" fill="white"></path>
              </mask>
              <g mask="url(#mask0_78_357)">
                <path d="M10.0034 3.90831L1.41176 12.5L0 11.0882L8.59169 2.49654H1.01905V0.5H12V11.481H10.0034V3.90831Z" fill="#004AAD"></path>
              </g>
            </g>
            <defs>
              <clipPath id="clip0_78_357">
                <rect width="12" height="12" fill="white" transform="translate(0 0.5)"></rect>
              </clipPath>
            </defs>
          </svg>
        </a>
      </div>

      <div class="row row-cols-xxl-4 row-cols-xl-4 row-cols-lg-4 row-cols-md-2 row-cols-sm-1 row-cols-1">
        <!-- Box1 -->
        <?php
            $user_id = get_current_user_id();

            $args = array(
              'role' => 'staff',
              'meta_key' => 'first_name',
              'orderby' => 'meta_value',
              'order' => 'ASC'
              // 'orderby' => 'ID',
              // 'order' => 'DESC',
            );
            $entries = get_users( $args );
            $total_query = count($entries);


              foreach ( $entries as $entry ) {
                $avatarUrl = '';
                 $avatarArray = get_user_meta( $entry->ID, 'wp_user_avatars' , true );
                 if(isset($avatarArray) &&  isset($avatarArray['full']) && $avatarArray['full']!=null && $avatarArray['full']!='' ){
                  $avatarUrl = $avatarArray['full'];
                 } else {
                  $avatarUrl = get_avatar_url( $entry->ID,['size' => '300'] );
                 }
      ?>
        <div class="col">
          <div class="lyc-members-box d-flex flex-column justify-content-between gap-xxl-4 gap-xl-4 gap-lg-3 gap-md-3 gap-sm-2 gap-2">
            <div class="lyc-members-box-img w-100">
              <img src="<?php echo $avatarUrl; ?>" class="img-fluid w-100" width="300" height="400" title="<?php echo $entry->user_firstname ; ?>" alt="<?php echo $entry->user_firstname ; ?>">
            </div>
            <div class="lyc-members-box-content d-flex flex-column justify-content-between gap-1">
              <h3><?php echo $entry->user_firstname ; ?> <?php echo $entry->user_lastname; ?></h3>
              <p><?php echo $entry->lyc_position ; ?></p>
            </div>
          </div>
        </div>
        <?php } ?>
      </div>


      <?php endwhile; ?>
      <?php endif; ?>
    </div>
  </section>
  <!-- Members -->
  <!-- Client -->
  <section class="lyc-clients py-xxl-7 py-xl-7 py-lg-5 py-md-5 py-sm-3 py-3">
    <div class="container">
      <?php if( have_rows('cilent_silder') ): ?>
      <?php while( have_rows('cilent_silder') ): the_row();

        ?>
      <div class="owl-carousel owl-theme lyc-client">
        <?php
            if (have_rows('silder')) : ?>
        <?php while (have_rows('silder')) : the_row();
                $image = get_sub_field('image');
              ?>
        <div class="item">
          <div class="lyc-client-box p-3 d-flex justify-content-center align-items-center bg-white">
            <img src="<?php echo $image['url']; ?>" width="127" height="104" title="<?php echo esc_attr($image['alt']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" class="img-fluid">
          </div>
        </div>
        <?php endwhile; ?>
        <?php endif; ?>
      </div>
      <?php endwhile; ?>
      <?php endif; ?>
    </div>
  </section>
  <!-- Client -->
  <!-- Testimonial -->

  <?php
    $args = array(
        'numberposts' => 5,
        'post_type'   => 'testimonials',
        'post_status'     => 'publish',
        'order' => 'ASC',
    );
    $testimonials = get_posts($args);
    if (count($testimonials) > 0) {
  ?>
  <section class="lyc-testimonial my-xxl-7 my-xl-7 my-lg-5 my-md-5 my-sm-3 my-3 position-relative">
    <div class="container">
      <div class="lyc-testimonial-heading d-flex justify-content-between align-items-center flex-wrap gap-3 mb-xxl-5 mb-xl-5 mb-lg-4 mb-md-4 mb-sm-3 mb-2 ">
        <div class="lyc-heading d-flex flex-column">
          <?php if( have_rows('testomonial') ): ?>
          <?php while( have_rows('testomonial') ): the_row();
              $title = get_sub_field('title');
              $sub_heading = get_sub_field('sub_heading');
              ?>
          <span><?php echo $title; ?></span>
          <h2><?php echo $sub_heading; ?></h2>
          <?php endwhile; ?>
          <?php endif; ?>
        </div>
        <a href="<?php echo site_url('testimonial'); ?>" class="btn btn-primary d-flex flex-wrap justify-content-center align-items-center gap-2">View all testimonials
          <svg width="12" height="13" viewBox="0 0 12 13" fill="none" xmlns="http://www.w3.org/2000/svg">
            <g clip-path="url(#clip0_78_357)">
              <mask id="mask0_78_357" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="12" height="13">
                <path d="M12 0.5H0V12.5H12V0.5Z" fill="white"></path>
              </mask>
              <g mask="url(#mask0_78_357)">
                <path d="M10.0034 3.90831L1.41176 12.5L0 11.0882L8.59169 2.49654H1.01905V0.5H12V11.481H10.0034V3.90831Z" fill="#004AAD"></path>
              </g>
            </g>
            <defs>
              <clipPath id="clip0_78_357">
                <rect width="12" height="12" fill="white" transform="translate(0 0.5)"></rect>
              </clipPath>
            </defs>
          </svg>
        </a>
      </div>
      <div class="lyc-testimonial-content">
        <div class="owl-carousel owl-theme lyc-testimonial-slide">
          <!-- Item1 -->
          <?php foreach ($testimonials as $testimonial) {  ?>
          <div class="item">
            <div class="lyc-testimonial-box h-100 d-flex justify-content-between flex-column gap-3">
              <div class="lyc-box-heading d-flex align-items-center gap-3">
                <span class="d-flex justify-content-center align-items-center bg-secondary"><svg xmlns="http://www.w3.org/2000/svg" width="40" height="28" viewBox="0 0 40 28" fill="none">
                    <path d="M16.3035 4.76716C15.2677 3.15558 13.7363 1.92342 11.9395 1.25587C10.1427 0.588315 8.17763 0.521435 6.33954 1.06529C4.50145 1.60914 2.88969 2.73433 1.74651 4.27175C0.603331 5.80918 -0.00950542 7.67577 0.000111478 9.59097C0.00106493 11.177 0.428299 12.7336 1.23718 14.0982C2.04607 15.4629 3.20692 16.5855 4.59845 17.3487C5.98999 18.112 7.56115 18.4879 9.14779 18.4372C10.7344 18.3865 12.2783 17.911 13.6182 17.0604C12.9216 19.127 11.6241 21.3317 9.48651 23.5418C9.07753 23.9645 8.85336 24.5322 8.86334 25.12C8.87331 25.7078 9.1166 26.2676 9.53968 26.6762C9.96277 27.0848 10.531 27.3088 11.1194 27.2988C11.7077 27.2888 12.268 27.0458 12.677 26.6231C20.5788 18.4417 19.5525 9.54847 16.3035 4.77779V4.76716ZM37.5735 4.76716C36.5377 3.15558 35.0063 1.92342 33.2095 1.25587C31.4127 0.588315 29.4476 0.521435 27.6095 1.06529C25.7714 1.60914 24.1596 2.73433 23.0165 4.27175C21.8733 5.80918 21.2604 7.67577 21.2701 9.59097C21.271 11.177 21.6982 12.7336 22.5071 14.0982C23.316 15.4629 24.4769 16.5855 25.8684 17.3487C27.2599 18.112 28.8311 18.4879 30.4177 18.4372C32.0044 18.3865 33.5483 17.911 34.8881 17.0604C34.1916 19.127 32.8941 21.3317 30.7565 23.5418C30.3475 23.9645 30.1233 24.5322 30.1333 25.12C30.1433 25.7078 30.3865 26.2676 30.8096 26.6762C31.2327 27.0848 31.8009 27.3088 32.3893 27.2988C32.9777 27.2888 33.538 27.0458 33.9469 26.6231C41.8487 18.4417 40.8225 9.54847 37.5735 4.77779V4.76716Z" fill="white" />
                  </svg></span>
                <div class="lyc-box-title d-flex flex-column gap-1">
                  <h3><?php echo $testimonial->post_title; ?></h3>
                  <p><?php echo $testimonial->post_excerpt; ?></p>
                </div>
              </div>
              <div class="lyc-box-content p-xxl-4 p-xl-4 p-lg-4 p-md-3 p-sm-3 p-3 w-100">
                <?php echo $testimonial->post_content; ?>
              </div>
            </div>
          </div>
          <?php } ?>

        </div>
      </div>
    </div>
    <div class="lyc-testimonial-bg position-absolute top-0 bottom-0 end-0 h-100">
      <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/media/testimonial/Vector.png" title="vector" alt="vector" class="img-fluid h-100">
    </div>
  </section>
  <?php } ?>
  <!-- cta -->
  <section class="lyc-cta bg-primary-500 position-relative pb-xxl-7 pb-xl-7 pb-lg-5 pb-md-5 pb-sm-3 pb-3 pt-xxl-11 pt-xl-11 pt-lg-10 pt-md-10 pt-sm-7 pt-7">
    <div class="container">
      <?php if( have_rows('head_over') ): ?>
      <?php while( have_rows('head_over') ): the_row();

        $title = get_sub_field('title');
        $content = get_sub_field('content');
        $button = get_sub_field('button');
        $image = get_sub_field('image');
        $every = get_sub_field('every');

        ?>
      <div class="row">
        <div class="col-xxl-8 col-xl-8 col-lg-6 col-md-12 col-sm-12 col-12 d-flex justify-content-center align-items-center">
          <div class="lyc-heading d-flex flex-column gap-xxl-4 gap-xl-4 gap-lg-3 gap-md-3 gap-sm-2 gap-2">
            <h2 class="text-white"><?php echo $title;  ?></h2>
            <p class="text-white"><?php echo $content;  ?></p>
            <!-- <a href="<?php //echo esc_url( $button['url'] ); ?>" class="btn btn-secondary d-flex align-items-center gap-2">
              <?php //echo esc_attr( $button['title'] ); ?>
              <svg width="12" height="13" viewBox="0 0 12 13" fill="none" xmlns="http://www.w3.org/2000/svg">
                <g clip-path="url(#clip0_78_357)">
                  <mask id="mask0_78_357" style="mask-type:luminance" maskUnits="userSpaceOnUse" x="0" y="0" width="12" height="13">
                    <path d="M12 0.5H0V12.5H12V0.5Z" fill="white"></path>
                  </mask>
                  <g mask="url(#mask0_78_357)">
                    <path d="M10.0034 3.90831L1.41176 12.5L0 11.0882L8.59169 2.49654H1.01905V0.5H12V11.481H10.0034V3.90831Z" fill="#004AAD"></path>
                  </g>
                </g>
                <defs>
                  <clipPath id="clip0_78_357">
                    <rect width="12" height="12" fill="white" transform="translate(0 0.5)"></rect>
                  </clipPath>
                </defs>
              </svg>
            </a> -->
          </div>
        </div>
        <div class="col-xxl-4 col-xl-4 col-lg-6 col-md-12 col-sm-12 col-12">
          <div class="lyc-cta-img position-relative d-flex justify-content-center align-items-center">
            <?php
                if (!empty($image)) : ?>
            <img src="<?php echo esc_url($image['url']); ?>" title="<?php echo esc_attr($image['alt']); ?>" alt="<?php echo esc_attr($image['alt']); ?>" class="img-fluid" width="500" height="500">
            <?php endif; ?>
            <!-- <span class="lyc-cta-in d-flex flex-column justify-content-center align-items-center gap-xxl-2 gap-xl-2 gap-lg-2 gap-md-1 gap-sm-1 gap-1 text-white p-3 position-absolute">
              <?php //echo esc_html($every);  ?>
              <?php
                  //$formatted_text = str_replace(['<p>', '</p>'], '', $every);
                //  echo esc_html($formatted_text);
                ?>
              <?php //echo htmlspecialchars_decode($formatted_text); ?>
                 <p class="text-white m-0">For every</p>
                  <h4 class="text-white">£1</h4>
                  <p class="text-white m-0">invested</p>
            </span> -->
          </div>
        </div>
      </div>
      <?php endwhile; ?>
      <?php endif; ?>
    </div>
    <div class="lyc-background position-absolute bottom-0 top-0">
      <img src="<?php echo get_stylesheet_directory_uri(); ?>/assets/media/cta/cta-bg.png" title="cta" alt="cta" class="img-fluid h-100" width="1920" height="660">
    </div>
  </section>
  <!-- cta -->

</main>











<?php
get_footer();
?>